        ...
        for i in range(num_food_sources):
            if trials[i] > max_trials:
                ...
                if len(frequent_itemsets)!=0: butility=int(frequent_itemsets[0][0][0])
                    ...
                    r3=1-r1
                    r2=0
                food_sources[i] = r1*np.random.uniform(-100,100, D)+r2*food_sources[butility]+r3*np.array(food_sources[i])
                '''
...
print(f"Mean Function Error UMABC_EO: {mean_error:.2E} ± {std_deviation:.2E}")
